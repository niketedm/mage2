...
], function ($, _, url, statesStorage, $t, chosen) {
    $.widget('mage.amLocator', {
        ...
        makeAjaxCall: function (sortByDistance, isReset) {
            ...
            $.ajax({
                ...
                //for home only one record is passing
                if (response && _.isArray(response.items) && self.options.homeHeader && self.options.commonBlock && self.options.homePage) {
        ...
        processLocation: function () {
            ...
            if (self.storeListIdentifier) {
                ...
                //for removing other store location html in common block
                if (self.options.commonBlock) {
                    var tryBeforeContent = '';
                    url.setBaseUrl(BASE_URL);
                    var firstStore = _.first(locations.items);
                    if( $.mage.cookies.get('custloc') != 'NA') {
                        nearestId = $.mage.cookies.get('nearestid');
                        distance = $.mage.cookies.get('distance');
                        _.each(locations.items,function(items){
                            if(items.id == nearestId){
                                var directionUrl = url.build('amlocator/' + items.url_key);
                                tryBeforeContent = '<div class="SL-tbb"><div class="addr-name"><div class="SL-name">' + items.name + '</div><div class="SL-address">' + items.address + ', <br>' + items.city + ', ' + items.state + ', ' + items.zip + '<br>Call:' + items.phone + '</div></div><div class="dist-dir"><div class="SL-distance">' + distance + ' Miles</div><div class="direc"><a href=' + directionUrl + '> Directions </a></div></div></div>';
                            }
                        });
                    } else {
                        if (firstStore.distance) {
                       
                            if (firstStore.distance < 101) {
                                var directionUrl = url.build('amlocator/' + firstStore.url_key);
                                tryBeforeContent = '<div class="SL-tbb"><div class="addr-name"><div class="SL-name">' + firstStore.name + '</div><div class="SL-address">' + firstStore.address + ', <br>' + firstStore.city + ', ' + firstStore.state + ', ' + firstStore.zip + '<br>Call:' + firstStore.phone + '</div></div><div class="dist-dir"><div class="SL-distance">' + firstStore.distance + ' Miles</div><div class="direc"><a href=' + directionUrl + '> Directions </a></div></div></div>';
                            } else {
                                tryBeforeContent = '<div class="SL-tbb no-loc">' + $t('<div>No locations found within 100 miles,</div><div>please call 800-647-5337 for the store nearest you.</div>') + '</div>'
                            }
                         
                        }
                    }
                    self.storeListIdentifier.html(tryBeforeContent);
                }
