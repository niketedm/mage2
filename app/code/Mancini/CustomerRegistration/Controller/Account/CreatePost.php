...
namespace Mancini\CustomerRegistration\Controller\Account;
...
class CreatePost extends AbstractAccount implements CsrfAwareActionInterface, HttpPostActionInterface
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            $customerEmailData = $this->_customerModel->getCollection()->addFieldToFilter('email', $email);
            $customerMobileData = $this->_customerModel->getCollection()->addFieldToFilter('mobile', $mobile);



            if(!count($customerEmailData) && !count($customerMobileData)) {
                $customer_details = $this->getRequest()->getParams();
                $result = $this->_apiCallHelper->inetWebsiteCreateAccountDetail($customer_details);
                $customer = $this->accountManagement->createAccount($customer, $password, $redirectUrl);

                if(!empty($result['token'])) {
                    //$customer = $this->accountManagement->createAccount($customer, $password, $redirectUrl);
                    $customerData   =   $this->_customerModel->load($customer->getId());
                    $customerData->setFrogtoken($result['token']);
                    $customerData->save();
                    $message = __('Thank you for registering with Main Website Store.');
                } else {
                    $message = __(
                        'Error creating account. Contact customer support to resolve this error.'
                    );
                }
            } else{
                $message = __(
            ...
            $this->messageManager->addSuccess(__('Thank you for registering with Main Website Store.'));
            ...
            $this->messageManager->addExceptionMessage($e, __('Error creating account. Contact customer support to resolve this error.'));
        }
