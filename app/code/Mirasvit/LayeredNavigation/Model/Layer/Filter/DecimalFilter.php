...
class DecimalFilter extends AbstractFilter
{
     ...
     * @return bool
     */
    //    protected function addState(string $label, $attributeValue)
    //    {
    ...
    private function renderRangeLabel(?float $fromPrice, ?float $toPrice): ?string
    {
        ...
        if ($this->_requestVar === 'price') {
            ...
            $fromText = str_replace('{value}', (string)round($fromPrice), $valueTemplate);
            $toText   = str_replace('{value}', (string)round($toPrice), $valueTemplate);
        }
